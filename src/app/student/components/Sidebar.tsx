...
export default function Sidebar({ onSelect }) {
  const links = [
    ...
    { name: 'Reports', icon: 'M5 12h14M12 5l7 7-7 7', href: '#' },
      ...
      <nav className="w-full">
        <ul className="flex flex-col px-6 gap-2 space-y-4">
          {links.map((link, index) => (
            <li key={index}>
              <Link href={link.href} onClick={() => onSelect(link.name)} className="flex items-center text-black dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition">
                <svg
